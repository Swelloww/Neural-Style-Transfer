...
# --- 训练逻辑 ---
content_weight, style_weight, tv_weight = 1, 5e5, 50
...
def train(content_path, style_path, num_epochs=500, lr=0.1, lr_decay_epoch=100):
...
if __name__ == "__main__":
    content_path = './img/rainier.jpg'
    style_path = './img/autumn-oak.jpg'
    
    if not os.path.exists(content_path) or not os.path.exists(style_path):
        print("错误：请确保 ./img/ 目录下有 rainier.jpg 和 autumn-oak.jpg")
    else:
        strat_time = time.time()
        output = train(content_path, style_path, num_epochs=2000, lr=0.1)
        final_img = postprocess(output)
        end_time = time.time()
        print(f"总训练时间: {end_time - strat_time:.2f} 秒")
        final_img.save("output/Gatys_stylized_output.png")
